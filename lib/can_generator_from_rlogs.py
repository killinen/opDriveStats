...
class MultiLogIterator(object):
    def __init__(self, log_paths, wraparound=False):
        ...
        self._first_log_idx = next(i for i in range(
            len(log_paths)) if log_paths[i] is not None)
...
if __name__ == "__main__":
    ...
    log_path = sys.argv[1]
    for filename in sorted(glob.iglob(log_path + '**/rlog.bz2', recursive=True), key=numericalSort):
        ...
        for msg in lr:
            # print(msg)
            #print("New msg")
            #print("Unix time :", msg.logMonoTime)
            tme = msg.logMonoTime
            #print(tme / 1e9)
            if first == True:
                tmeo = tme
        ...
        fw = msg.carState.vEgo
        print(fw)
        print(type(fw))'''
            if msg.which() == 'can':
                #print("Timestamp:", tme)
                #print("Timestamp:", tme / 100000, "secs")
                #print("Old timestamp:", tmeo)
                #t_diff = (tme - tmeo) / 100000
                #print("Time difference:", t_diff, "ms")
                #tmeo = tme
                # print(msg.which())
                #can = msg.can
                # print(can)
                ...
                for c in msg.can:
                    # msgs[c.address]
                    #print("Timestamp:", tme)
                    #print("Can message address in HEX:", hex(c.address), "Data in HEX:",  c.dat.hex(),"Data length:", len(c.dat.hex()))
                    ################################## This is for printing for full CAN log ##############################################
                    # print("(", format((tme / 1e9), '.6f')[-14:], ")", " can0 ", hex(
                    #     c.address)[-3:].upper(), "#", c.dat.hex().upper(), sep='')
                    ## break
                    ########################## If want to filter certain msgs this is how to do it ########################################
                    #if c.address != 466 or c.address != 467:
                    if not c.address in [0x1D2, 0x1D3]:
                        #   print(c.address)
                        print("(", format((tme / 1e9), '.6f')[-14:], ")", " can0 ", hex(
                            c.address)[-3:].upper(), "#", c.dat.hex().upper(), sep='')
            #tmeo = tme
